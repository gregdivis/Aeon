...
{
    private readonly unsafe uint[] palette = new uint[256];
    ...
    /// </summary>
    public ReadOnlySpan<uint> Palette => this.palette;
    ...
    public byte Read()
    {
        uint color = new UnsafePointer<uint>(this.palette)[this.readIndex];
        this.readChannel++;
    ...
    {
        ref uint entry = ref new UnsafePointer<uint>(this.palette)[this.writeIndex];
        this.writeChannel++;
        if (this.writeChannel == 1)
        {
            entry &= 0xFF00FFFF;
            entry |= (uint)((value & 0x3F) << 18);
        }
        else if (this.writeChannel == 2)
        {
            entry &= 0xFFFF00FF;
            entry |= (uint)((value & 0x3F) << 10);
        }
        else
        {
            entry &= 0xFFFFFF00;
            entry |= (uint)((value & 0x3F) << 2);
            this.writeChannel = 0;
            this.writeIndex++;
        }
    }
    ...
    public void Reset()
    {
        ...
        var dest = new UnsafePointer<uint>(this.palette);
        for (int i = 0; i < 256; i++)
        {
            uint r = source[i * 3];
            uint g = source[i * 3 + 1];
    ...
    public void SetColor(byte index, byte r, byte g, byte b)
    {
        ...
        new UnsafePointer<uint>(this.palette)[index] = red | green | blue;
    }

    #region DefaultPalette
    private static ReadOnlySpan<byte> DefaultPalette =>
    [
