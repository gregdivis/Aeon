...
{
    private static readonly Dictionary<string, OperandType> operandStrings = [];
    private static readonly Dictionary<int, OperandFormat> operandFormats = [];
    ...
    public readonly int RPlusIndex
    {
        get
        {
            if (ModRmByte == ModRmInfo.RegisterPlus)
                return this.Operands.IndexOfAny([OperandType.RegisterST, OperandType.RegisterByte, OperandType.RegisterWord]);
    ...
    public readonly bool IsAffectedByOperandSize => this.Operands.IndexOfAny([OperandType.ImmediateFarPointer, OperandType.ImmediateRelativeWord, OperandType.ImmediateWord, OperandType.IndirectFarPointer, OperandType.MemoryOffsetWord, OperandType.RegisterOrMemoryWord, OperandType.RegisterOrMemoryWordNearPointer, OperandType.RegisterWord, OperandType.EffectiveAddress, OperandType.FullLinearAddress]) >= 0;
    public readonly bool IsAffectedByAddressSize => this.Operands.IndexOfAny([OperandType.IndirectFarPointer, OperandType.MemoryInt16, OperandType.MemoryInt32, OperandType.MemoryFloat32, OperandType.MemoryFloat64, OperandType.MemoryFloat80, OperandType.MemoryInt64, OperandType.MemoryOffsetByte, OperandType.MemoryOffsetWord, OperandType.RegisterOrMemory16, OperandType.RegisterOrMemory32, OperandType.RegisterOrMemoryByte, OperandType.RegisterOrMemoryWord, OperandType.RegisterOrMemoryWordNearPointer, OperandType.EffectiveAddress, OperandType.FullLinearAddress]) >= 0;

    public static InstructionInfo Parse(string instructionFormat)
    {
        var info = new InstructionInfo();

        var outer = instructionFormat.Split(' ', StringSplitOptions.RemoveEmptyEntries);

        string[] innerCode;
        if (!outer[0].Contains('+'))
            innerCode = outer[0].Split('/', StringSplitOptions.RemoveEmptyEntries);
        else
            innerCode = [outer[0].TrimEnd('+'), "+"];
        ...
        if (outer.Length > 1)
        {
            string[] innerOperands = outer[1].Split(',', StringSplitOptions.RemoveEmptyEntries);
    ...
    public readonly IEnumerable<InstructionInfo> Expand()
    {
    ...
    public override readonly int GetHashCode() => this.Opcode | this.ExtendedRmOpcode << 16;
