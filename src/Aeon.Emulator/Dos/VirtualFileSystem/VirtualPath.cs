...
public sealed class VirtualPath : IEquatable<VirtualPath>
{
    ...
    public VirtualPath(string path) : this()
    {
        ...
        var instance = Parse(path, out var error) ?? throw error!;
    ...
    private VirtualPath(string[] elements, VirtualPathType type, DriveLetter? drive)
        : this()
    {
        this.pathElements = elements.Length > 0 ? elements : [];
        this.pathType = type;
    ...
    public VirtualPath GetRelativePath(VirtualPath pathTo)
    {
        ...
        else
        {
            ...
            return new VirtualPath([.. buffer], VirtualPathType.Relative, null);
        }
    }
    /// <summary>
    ...
    {
        ArgumentNullException.ThrowIfNull(path);
        return new VirtualPath([], VirtualPathType.Absolute, driveLetter).Append(path);
    }
    /// <summary>
    ...
    private static VirtualPath? Parse(ReadOnlySpan<char> path, out Exception? error)
    {
        ...
        else
        {
            ...
            var elements = path.ToString().Split(['\\', '/'], StringSplitOptions.RemoveEmptyEntries);
