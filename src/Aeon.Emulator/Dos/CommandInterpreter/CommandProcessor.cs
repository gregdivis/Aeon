...
internal sealed partial class CommandProcessor(VirtualMachine vm)
{
    ...
    private string ReplaceVariables(string s)
    {
        ...
        var result = NumberedArgRegex().Replace(
            s,
            m =>
            {
                var b = this.CurrentBatch;
        ...
        result = EnvironmentVariableRegex().Replace(
            result,
            m =>
            {
    ...
    [GeneratedRegex(@"%(?<1>[0-9]+)", RegexOptions.Singleline | RegexOptions.ExplicitCapture)]
    private static partial Regex NumberedArgRegex();
    [GeneratedRegex(@"%(?<1>[^\s%]+)%", RegexOptions.Singleline | RegexOptions.ExplicitCapture)]
