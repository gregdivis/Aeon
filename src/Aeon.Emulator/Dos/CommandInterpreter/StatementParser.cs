...
public static class StatementParser
{
    ...
    private static CommandStatement? ParseInternal(ReadOnlySpan<char> s, out bool noEcho)
    {
        ...
        // :label
        if (trimmed[0] == ':')
            return ParseLabel(trimmed[1..].Trim());
        ...
        if (trimmed.StartsWith("cd", StringComparison.OrdinalIgnoreCase) && trimmed.Length > 2 && (char.IsWhiteSpace(trimmed[2]) || trimmed[2] == '\\' || trimmed[2] == '.'))
            return new SetCurrentDirectoryCommand(trimmed[2..].Trim().ToString());

        Split(s.TrimStart()[(noEcho ? 1 : 0)..], out var commandName, out var args);
    ...
    private static CommandStatement ParseIfEquals(bool not, ReadOnlySpan<char> args)
    {
        ...
        var remainder = args[(value1.Length + 2)..].TrimStart();
        ...
        var command = Parse(remainder[(value2.Length + 2)..].TrimStart());
        if (command == null)
