using System.Runtime.CompilerServices;
using System.Runtime.InteropServices;
using Aeon.Emulator.Memory;

namespace Aeon.Emulator;

/// <summary>
/// Contains information about a protected mode segment.
/// </summary>
[StructLayout(LayoutKind.Sequential, Pack = 1, Size = 8)]
public readonly struct SegmentDescriptor
{
    /// <summary>
    /// Big word mode in Attributes2.
    /// </summary>
    public const uint BigMode = 1 << 6;
    /// <summary>
    /// 4KB granularity mode in Attributes2.
    /// </summary>
    public const uint Granularity = 1 << 7;
    /// <summary>
    /// Indicates whether the segement is a code segment (1) or a data segment (0).
    /// </summary>
    public const uint CodeData = 1 << 3;
    /// <summary>
    /// For a code segment: 0 = XR, 1 = R. For a data segment: 0 = R, 1 = RW.
    /// </summary>
    public const uint ReadWrite = 1 << 1;
    /// <summary>
    /// Present flag in Attributes1.
    /// </summary>
    public const uint Present = 1 << 7;

    private readonly ushort limit1;
    private readonly ushort base1;
    private readonly byte base2;
    private readonly byte attributes1;
    private readonly byte attributes2;
    private readonly byte base3;

    public SegmentDescriptor()
    {
        this.limit1 = 0;
        this.base1 = 0;
        this.base2 = 0;
        this.attributes1 = 0b0001_0000;
        this.attributes2 = 0;
        this.base3 = 0;
    }

    /// <summary>
    /// Casts a segment descriptor to a descriptor.
    /// </summary>
    /// <param name="descriptor">Segment descriptor to cast.</param>
    /// <returns>Resulting descriptor.</returns>
    public static implicit operator Descriptor(SegmentDescriptor descriptor) => Unsafe.As<SegmentDescriptor, Descriptor>(ref descriptor);
    public static explicit operator ulong(SegmentDescriptor descriptor) => Unsafe.As<SegmentDescriptor, ulong>(ref descriptor);

    /// <summary>
    /// Gets or sets the physical base address of the segment.
    /// </summary>
    public uint Base => this.base1 | ((uint)this.base2 << 16) | ((uint)this.base3 << 24);

    /// <summary>
    /// Gets or sets the size of the segment.
    /// </summary>
    public uint Limit => this.limit1 | ((this.attributes2 & 0x0Fu) << 16);

    /// <summary>
    /// Gets attribute byte 1 of the descriptor.
    /// </summary>
    public readonly byte Attributes1 => attributes1;
    /// <summary>
    /// Gets attribute byte 2 of the descriptor.
    /// </summary>
    public readonly byte Attributes2 => attributes2;
    /// <summary>
    /// Gets the privilege level of the descriptor.
    /// </summary>
    public readonly uint PrivilegeLevel => (uint)(this.attributes1 >> 5) & 0x3u;
    /// <summary>
    /// Gets the limit of the descriptor in bytes.
    /// </summary>
    public readonly uint ByteLimit => (this.attributes2 & Granularity) == 0 ? this.Limit : (this.Limit << 12) | 0xFFFu;
    /// <summary>
    /// Gets or sets a value indicating whether the descriptor descrbies a code segment.
    /// </summary>
    public bool IsCodeSegment => (this.Attributes1 & CodeData) != 0;

    /// <summary>
    /// Gets or sets a value indicating whether the segment can be read.
    /// </summary>
    public bool CanRead => !this.IsCodeSegment || (this.Attributes1 & ReadWrite) != 0;
    /// <summary>
    /// Gets a value indicating whether the segment can be written to.
    /// </summary>
    public readonly bool CanWrite => !this.IsCodeSegment && (this.Attributes1 & ReadWrite) != 0;
    /// <summary>
    /// Gets or sets a value indicating whether the segment is present.
    /// </summary>
    public bool IsPresent => (this.attributes1 & Present) != 0;
    /// <summary>
    /// Gets or sets a value indicating whether page granularity mode is enabled.
    /// </summary>
    public bool PageGranularity => (this.attributes2 & Granularity) != 0;
}
