...
/// </summary>
internal readonly struct XmsBlock(int handle, uint offset, uint length, bool used) : IEquatable<XmsBlock>
{
    /// <summary>
    ...
    public XmsBlock[] Allocate(int handle, uint length)
    {
        ...
        ArgumentOutOfRangeException.ThrowIfGreaterThan(length, this.Length);

        if (length == this.Length)
            return [new XmsBlock(handle, this.Offset, length, true)];
    ...
    public XmsBlock Free() => new(0, this.Offset, this.Length, false);
    /// <summary>
